            ...
            for j, col in enumerate(pvalue_matrix.columns):
                ...
                if np.isnan(synergy):
                    ...
                    hover_row.append(
                        ...
                        f"<b>Counts (y=0)</b>: ab={counts.get('Count_00|y=0', 0)}, "
                        f"aB={counts.get('Count_01|y=0', 0)}, "
                        f"Ab={counts.get('Count_10|y=0', 0)}, "
                        f"AB={counts.get('Count_11|y=0', 0)} (N={counts.get('N_y0', 0)})<br>"
                        f"<b>Counts (y=1)</b>: ab={counts.get('Count_00|y=1', 0)}, "
                        f"aB={counts.get('Count_01|y=1', 0)}, "
                        f"Ab={counts.get('Count_10|y=1', 0)}, "
                        f"AB={counts.get('Count_11|y=1', 0)} (N={counts.get('N_y1', 0)})<br>"
