...
def large_hpo_data():
    ...
    pmids_matrix = pd.DataFrame({
        "pmids": [list(np.random.choice(["PM1", "PM2", "PM3"], size=np.random.randint(1,3), replace=False))
                  for _ in range(n_samples)]
    }, index=hpo_matrix.index)
    
    relationship_mask = pd.DataFrame(np.zeros((4,4)), index=hpo_matrix.columns, columns=hpo_matrix.columns)
    
    return (hpo_matrix, relationship_mask, pmids_matrix)

@pytest.fixture
def large_target():
    n_samples = 40
    return pd.Series(np.random.randint(0, 2, size=n_samples), index=[f"P{i+1}" for i in range(n_samples)])

def test_initialize(large_hpo_data, large_target):
    analyzer = PairwiseSynergyAnalyzer(large_hpo_data, large_target, n_permutations=10)
...
def test_filter_weak_synergy(large_hpo_data, large_target):
    ...
    filtered_synergy, filtered_p = analyzer.filter_weak_synergy(lower_bound=0.0, alpha=1.0)
...
def test_plot_synergy_heatmap(large_hpo_data, large_target):
    analyzer = PairwiseSynergyAnalyzer(large_hpo_data, large_target, n_permutations=10)
    analyzer.compute_synergy_matrix(n_jobs=1)
    fig = analyzer.plot_synergy_heatmap(lower_bound=0.0, target_name="Test Target")
...
def test_save_synergy_heatmap(tmp_path, large_hpo_data, large_target):
    analyzer = PairwiseSynergyAnalyzer(large_hpo_data, large_target, n_permutations=10)
    analyzer.compute_synergy_matrix(n_jobs=1)
    fig = analyzer.plot_synergy_heatmap(lower_bound=0.0)
